...
file_directory <- "data"
file_prefix <- "CCGOM_raw" #The part before ".vcf"
popmap_file <- "popmap" #The part before ".txt"
...
**ISG: Think I've got this one! I've zoomed the plot in just a bit so that I can visualize singletons a bit more easily. A bit conservative filtering for H<0.55, can bump up to 0.6 if you'd suggest doing so. -2.5<D<2.5 seems reasonable, if a bit conservative, for getting rid of duplicate loci.**
```{r, echo=FALSE}
...
#Create heterozygosity vs read ratio deviation plot (ISG adjusted Y scale)
paralog_calcs %>% 
  ggplot(aes(x=H,y=D))+
  geom_point(alpha=0.06) +
  theme_classic()+
  scale_y_continuous(name = "D", limits=c(-20,20), breaks = seq(-20, 20, by = 2)) +
  scale_x_continuous(name = "H", breaks = seq(0, 1, by = 0.05)) +
  ggtitle("Heterozygosity (H) vs Read Ratio Deviation (D)") +
  geom_hline(yintercept = c(-3, 3), color = "blue", linetype = "dashed", linewidth = 0.4)
...
**ISG: A bit less confident about this. First graph (3 panel) shows that there are a lot of loci out of equilibrium, but this is unfiltered data so that's expected. Mean HMR is 0.044, which isn't too bad from what I've read (for example, HMR=0.02 is "low" according to Hendricks et al. 2018, "Recent advances in conservation and population genomics data analysis"). Second graph shows that HMR is highest in read depth bins <~10, with a slight peak between 25 and 50. Leaving the min mean depth at 12.5 seems reasonable, based on what I understand.**
```{r}
...
**ISG: Not very confident about this.**
**ISG: Step 1: Is 2m = 2*mean number of missing individuals per locus? If so, m=3.11 and 6 is the suggested threshold**
**ISG: Step 2: Not exactly sure where the cutoff for missing data per individual should be based on the frequency spectrum. 70 (already in the Rmd) seems reasonable, especially since there is more filtering downstream**
**ISG: Step 3: Iterative filtering parameters. As per O'Leary, Shannon J., et al. "These aren't the loci you'e looking for: Principles of effective SNP filtering for molecular ecologists." (2018): 3193-3206., hard cutoffs of imiss (allowed missing data per individual; <25%) and lmiss (allowed missing individuals per locus; <95%) at end of iterative filtering should optimally retain individuals and loci.**
```{r}
...
Imiss <- 25 #After removing failed loci, filter individuals that still have this much missing data or more
Lmiss <- 5 #After removing failed individuals, remove loci that are missing in this percent of individuals
...
**ISG: Not too certain what to set HWE_inquire to. Setting very low (e.g., 0.000001) removes just a few hundred loci. Setting fairly high (e.g., 0.05) removes ~16k. Currently set to 0.001. The Gulf of Mexico populations are based solely on differing mtDNA haplotype frequencies and exhibit nuclear (microsat) connectivity, so I suspect that stringent filtering for HWE would remove potentially informative loci at this stage? Appreciate your thoughts and suggestions!**
**ISG: Also, this is where I noticed the outlier individual from CWFL-will remove upstream of VCF generation once computing core is back up**
```{r}
...
**ISG: all remaining individuals are below max_het**
...
keep_hetero_indiv <- gt_tidy_filt2 %>% 
  anti_join(high_het_samples, by = "Indiv") %>%
  count(Indiv)

#Mark individuals with heterozygosity levels above the threshold
gt_tidy_hetero_remove <- gt_tidy_filt2 %>% 
  semi_join(high_het_samples, by = "Indiv")
...
gt_tidy_filt_final <- gt_tidy_filt2 %>% 
  ...
  dplyr::select(ChromKey, CHROM, POS, Indiv, gt_DP, gt_RO, gt_QR, gt_AO, gt_QA, gt_GL, gt_GT, gt_GT_alleles, allele_A, allele_B, zygosity, REF, ALT)#LMK: When the LD_loci_to_keep line is commented out, this line gives an error that ChromKey isn't found; may need to be adjusted for ref based/no short LD filtering option
...
**ISG: Temporary fix, manually removing outlier sample from whitelist**
**ISG: In Pop Gen Script, using gl.keep functions to keep whitelisted individuals and loci**
```{r}
...
#Export remaining loci to whitelist text file for filtering original VCF
fix_tidy_filt_final %>% dplyr::select(CHROM, POS) %>% 
