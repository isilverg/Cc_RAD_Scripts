...
HDplot<-function(vcfData){
  ...
  genos[is.na(genos)]<-"./."
  genos<-gsub("\\.", "./.", genos)
  genos<- replace(x = genos, 
                     list =  !genos %in% c("0/0","1/1","./.","0/1","1/0"), 
                     values =  './.') #replaces multi-allelic loci with "./."
  
  #get allele reads from vcf file
  #reads<-extract.gt(vcfData, element = "AD", mask = FALSE, as.numeric = FALSE, return.alleles = FALSE, 
                    #IDtoRowNames = TRUE, extract = TRUE, convertNA = FALSE)
  alleleReads1<-extract.gt(vcfData, element = "RO", mask = FALSE, as.numeric = FALSE, return.alleles = FALSE,
                           IDtoRowNames = TRUE, extract = TRUE, convertNA = FALSE)
  alleleReads2<-extract.gt(vcfData, element = "AO", mask = FALSE, as.numeric = FALSE, return.alleles = FALSE,
                           IDtoRowNames = TRUE, extract = TRUE, convertNA = FALSE)
  #replace reads for samples with missing data with 0
  #reads<-gsub("\\.,\\.","0,0",reads)
  #reads[grepl("\\.",reads)]<-"0,0"
  #reads[is.na(reads)]<-"0,0"
  alleleReads1[grepl("\\.",alleleReads1)]<-"0"
  alleleReads1[is.na(alleleReads1)]<-"0"
  alleleReads1[grepl(".,.", alleleReads1)]<-"0"
  alleleReads2[grepl("\\.",alleleReads2)]<-"0"
  alleleReads2[is.na(alleleReads2)]<-"0"
  alleleReads2[grepl(".,.", alleleReads2)]<-"0"
  ...
  alleleReads1<-apply(alleleReads1,2, function(x) as.numeric(x))
  alleleReads2<-apply(alleleReads2,2, function(x) as.numeric(x))
  ...
  alleleReads_1_het<-alleleReads1*hetMatrix
  alleleReads_2_het<-alleleReads2*hetMatrix
  #rows are loci and columns are samples
  #sum reads per allele per locus for heterozygous samples
  A_reads<-apply(alleleReads_1_het,1,sum, na.rm=TRUE)
  B_reads<-apply(alleleReads_2_het,1,sum, na.rm=TRUE)
  ...
  numHets<-apply(hetMatrix,1,sum, na.rm=TRUE)
