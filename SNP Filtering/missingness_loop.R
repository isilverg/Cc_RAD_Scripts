...
file_directory <- "data"
file_prefix <- "CCGOM_raw"
popmap_file <- "popmap"

popmap <- read_tsv(paste0(paste0(file_directory,"/", popmap_file, ".txt")), col_names = FALSE) %>% rename(Indiv = X1, Pop = X2)
...
write.csv(stats.df, file = "Filter_test.csv")
