...
popmap.orig <- read_tsv("../Combined_sort_OTHER_NRU_popmap.txt", col_names = F) %>% 
...
OTHERinds<-genlight_2@ind.names[grep(c("^(CEFL|SEFL|DRTO|SWFL|CWFL|NGM)"), genlight_2@ind.names)]

OTHER_final_inds <- sample(OTHERinds, 26, replace = FALSE)

final_inds<-c(NRUinds, OTHER_final_inds)
...
save(combinations_list, file = "Other_NRU_combinations_list.RData")
