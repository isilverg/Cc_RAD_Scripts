...
#Subset PFL and NRU due to larger sample size (all pops will have 18 individuals)
NRUinds<-genlight_2@ind.names[grep("^NRU", genlight_2@ind.names)]
PFLinds<-genlight_2@ind.names[grep(c("^(CEFL|SEFL|SWFL|CWFL)"), genlight_2@ind.names)]
DRTOinds<-genlight_2@ind.names[grep("^DRTO", genlight_2@ind.names)]
NGMinds<-genlight_2@ind.names[grep("^NGM", genlight_2@ind.names)]

NRU_final_inds <- sample(NRUinds, 18, replace = FALSE)
PFL_final_inds <- sample(PFLinds, 18, replace = FALSE)

final_inds<-c(NRU_final_inds, PFL_final_inds, DRTOinds, NGMinds)
...
save(combinations_list, file = "RU_combinations_list.RData")
